...
// --- 网格图常量定义 ---
// (5,7)_8 卷积码, G(D) = [1+D+D^2, 1+D^2] -> [111, 101] -> (7, 5) 八进制
// 注意：学长的编码器实现是 (7,5) 码，但状态表是 (5,7) 码。
// 我们这里统一使用 (5,7) 码，即 G(D) = [1+D^2, 1+D+D^2] -> [101, 111]
// 这对应编码器：c1 = m[i] ^ s0; c2 = m[i] ^ s0 ^ s1;
// 为了匹配学长的 viterbit.c，我们使用他定义的 (7,5) 码的网格图
// 编码器 (main.c 中): c1 = m[i]^s0^s1; c2 = m[i]^s1;
// 状态 = (s1, s0)

// 4种可能的输出 (c1, c2)
const int state[4][2] = { {0,0},{0,1},{1,0},{1,1} }; 
// 为输出(c00, c01, c10, c11) 定义索引
const int c00 = 0, c01 = 1, c10 = 2, c11 = 3; 
